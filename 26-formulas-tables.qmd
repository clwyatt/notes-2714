# Useful Mathematical Definitions and Tables {.unnumbered}
...
$$n \% N = \left\{ \begin{array}{cc}
  ...
  \end{array}
\right.$$ where $\text{remainder}$ is the remainder after dividing $n$
by $N$.

## Table of Representative Convolution Integrals {#table:ctconv}
...
## Table of Representative Convolution Sums {#table:dtconv}
...
## Table of Representative CT Fourier Transform Pairs {#table:ctft}
  ...
  ----------------------------------------------------------- ---------------------------------------------------------------------------------------------------------
   ...
   $\sum\limits_{k=-\infty}^{\infty} a_k e^{j k \omega_0 t}$                 $2 \pi \sum\limits_{k=-\infty}^{\infty} a_k \delta{(\omega - k \omega_0)}$
       $\sum\limits_{n=-\infty}^{\infty} \delta(t - nT)$       $\omega_0 \sum\limits_{k=-\infty}^{\infty} \delta{(\omega - k \omega_0)}$, $\omega_0 = \frac{2 \pi}{T}$

  : CT Fourier Transform Table

## Table of Representative DT Fourier Transform Pairs {#table:dtft}
  ...
  -------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------
                ...
                $\frac{\sin{({W}n)}}{\pi n}$                                $\sum\limits_{k = - \infty}^{\infty} X_1(\omega - 2 k \pi), X_1(\omega) = \begin{array}{cc} 1 & 0 \le |\omega| \le W, 0 < W < \pi\\ 0 & W < |\omega| \le \pi,  0 < W < \pi \end{array}$
   $\sum\limits_{k=n_0}^{n_0+N-1} a_k e^{j k \omega_0 n}$          $2 \pi \sum\limits_{k=-\infty}^{\infty} a_k \delta{(\omega - k \omega_0)}$, $\omega_0 = \frac{2 \pi}{N}$,
     $\sum\limits_{k=-\infty}^{\infty} \delta[n - kN]$                $\omega_0 \sum\limits_{k=-\infty}^{\infty} \delta{(n - k \omega_0)}$, $\omega_0 = \frac{2 \pi}{N}$,
