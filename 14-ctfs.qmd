...
of the Eigenfunction becomes $s_k = jk\omega_0$, a complex sinusoid with frequencies that are harmonics of the fundamental frequency. The decomposition
...
:::{.example}
::::{.example-header}
### Example of the Fourier series. {.unnumbered}
::::
::::{.example-container}
...
:::{.example}
::::{.example-header}
### Fourier series of the impulse train. {.unnumbered}
::::
::::{.example-container}
...
:::{.example}
::::{.example-header}
### Fourier series of a sinusoid. {.unnumbered}
::::
::::{.example-container}
...
:::{.example}
::::{.example-header}
### Fourier series of a square wave. {.unnumbered}
::::
::::{.example-container}
...
plotted below for $A = 1$ and $T_0 = 1$ ($\omega_0 = 2\pi$).

```{python}
#| label: fig-14-example2
#| fig-cap: "Amplitude and phase spectrum plot for the example."
#| fig-alt: "Two stacked stem plots: the top shows the amplitude spectrum of a(k); the bottom shows the phase spectrum."
#| warning: false
#| echo: false
...
k = np.arange(-10, 11)
a = np.where(k % 2 == 0,
             0,                         # value when k is even
             2 / (1j * np.pi * k))      # value when k is odd
...
$$
x(t) \approx \sum\limits_{k = -N}^{N} a_k \, e^{j 2\pi k t}
$$
as well as the squared error over one period, and the compute the total error.
...
```{python}
#| label: fig-14-example3
#| fig-cap: "Truncated Fourier series plot for increasing number of terms, with square error, and total error. Blue is the original function, orange is the approximation."
#| fig-alt: "See Caption."
#| warning: false
#| echo: false
...
ftrue = np.where(t < 0.5, 1, -1)

# Plot approximation for 1 term
fapprox = (4/np.pi)*np.sin(2*np.pi*t)
ferr = ftrue-fapprox
totalerr = dt*np.sum(ferr*ferr)
...
plt.xlim(0, 1)
plt.xlabel('t')
plt.title('Approximation when N=1')

plt.subplot(3, 2, 2)
plt.plot(t, ferr*ferr)
plt.xlim(0, 1)
plt.xlabel('t')
plt.title(f"Square error when N=1 (total = {totalerr:.2f})")


# Plot approximation for 2 terms
fapprox = (4/np.pi)*np.sin(2*np.pi*t) + (4/(3*np.pi))*np.sin(6*np.pi*t)
ferr = ftrue-fapprox
totalerr = dt*np.sum(ferr*ferr)
...
plt.xlim(0, 1)
plt.xlabel('t')
plt.title('Approximation when N=3')

plt.subplot(3, 2, 4)
plt.plot(t, ferr*ferr)
plt.xlim(0, 1)
plt.xlabel('t')
plt.title(f"Square error when N=3 (total = {totalerr:.2f})")

# Plot approximation for 3 terms
fapprox = (4/np.pi)*np.sin(2*np.pi*t) + (4/(3*np.pi))*np.sin(6*np.pi*t) + + (4/(5*np.pi))*np.sin(10*np.pi*t)
ferr = ftrue-fapprox
totalerr = dt*np.sum(ferr*ferr)
...
plt.xlim(0, 1)
plt.xlabel('t')
plt.title('Approximation when N=5')

plt.subplot(3, 2, 6)
plt.plot(t, ferr*ferr)
plt.xlim(0, 1)
plt.xlabel('t')
plt.title(f"Square error when N=5 (total = {totalerr:.2f})")
...
except at the discontinuities. At the discontinuities the square error is bounded by one and gets narrower as $N$ increases, but never disappears entirely. This phenomena is called *Gibbs Ringing*. However, as
$N \rightarrow \infty$ the total mean-square error goes to zero, so the CTFS
...
- **Linearity**. The coefficients of the signal
  $$z(t) = Ax(t) + By(t) \mbox{ for constants } A,B$$ are $Aa_k + Bb_k$. This forms the basis of analysis using the Fourier series and the system Eigenvalues.
